...
def get_pie_chart(entered_site):
     ...
     if entered_site=='ALL':
              ...
              names='Launch Site',     
              title=f'Total Success Launch Sites')     
              return fig
     else:    
         filtered_df=spacex_df[spacex_df['Launch Site']==entered_site]     
         filtered_df1=filtered_df.groupby(['Launch Site','class']).size().reset_index(name='class count')               
         fig = px.pie(filtered_df1, values='class count', names='class', title=f'Total Success Launch Sites {entered_site}')    
         return fig
...
def get_scatter(entered_site,payload):
    ...
    if entered_site == 'ALL':
        fig = px.scatter(filtered_df2, 
                        ...
                        title=f'Payload vs Successful Launch for all Sites')
        ...
        fig = px.scatter(filtered_df3, 
                        ...
                        y='class', 
                        color='Booster Version Category', 
                        title=f'Payload vs Successful Launch for all Sites {entered_site}')
        return fig
